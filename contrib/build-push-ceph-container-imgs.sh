...
function compare_docker_hub_and_github_tags {
  ...
  for tag_github in $(git ls-remote --tags | grep -oE 'v[3-9].[0-9]*.[0-9]*$' | sort -t. -k 1,1n -k 2,2n -k 3,3n -k 4,4n); do
  ...
  tags_docker_hub_unsorted="$(./cn image ls -a | grep -oE 'v[3-9].[0-9]*.[0-9]' | uniq)"
  tags_docker_hub_sorted="$(echo "$tags_docker_hub_unsorted" | sort -t. -k 1,1n -k 2,2n -k 3,3n -k 4,4n)"
  for tag_docker_hub in $tags_docker_hub_sorted; do
  ...
  for i in "${tags_github_array[@]}"; do
    skip=
    for j in "${tags_docker_hub_array[@]}"; do
      [[ "$i" == "$j" ]] && { skip=1; break; }
    done
    [[ -n $skip ]] || tag_to_build+=("$i")
  done
